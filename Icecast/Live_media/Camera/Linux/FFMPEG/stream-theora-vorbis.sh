...
# ffmpeg -f alsa -i default -f video4linux2 -s 640x480 -r 30 -i /dev/video0 -c:a vorbis -t:a 128k -ice_name "$channel.ogg" -f ogg icecast://source:$2@$addr:$port/$channel.ogg
# ffmpeg -f x11grab -framerate 25 -video_size cif -i :0.0 -codec:v libtheora -qscale:v 10 -codec:a libvorbis -qscale:a 5 -f ogg - | oggfwd $addr $port $password /$channel
...
#ffmpeg -f alsa -i default -f video4linux2 -s 640x480 -r 15 -i /dev/video0 -codec:v libtheora -qscale:v 10 -codec:a libvorbis -qscale:a 5 -f ogg - | oggfwd $addr $port $password /$channe
echo ffmpeg -f alsa -i default -f video4linux2 -s 640x480 -r 15 -i /dev/video0 -codec:v libtheora -qscale:v 10 -codec:a libvorbis -qscale:a 5 -content_type application/ogg -f ogg icecast://source:$password@$addr:$port/$channel
ffmpeg -f alsa -i default -f video4linux2 -s 640x480 -r 15 -i /dev/video0 -codec:v libtheora -qscale:v 10 -codec:a libvorbis -qscale:a 5 -content_type application/ogg -f ogg icecast://source:$password@$addr:$port/$channel
